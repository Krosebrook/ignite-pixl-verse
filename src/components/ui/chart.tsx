...
function sanitizeCssColor(color: string): string | null {
  ...
  for (const pattern of dangerousPatterns) {
    if (pattern.test(trimmed)) {
      console.warn(`[ChartStyle] Blocked potentially malicious color value: ${trimmed.substring(0, 50)}`);
      return null;
    }
  }
  ...
  console.warn(`[ChartStyle] Invalid color format rejected: ${trimmed.substring(0, 50)}`);
  return null;
}
...
function sanitizeCssKey(key: string): string | null {
  ...
  if (!/^[a-zA-Z][a-zA-Z0-9_-]*$/.test(key)) {
    console.warn(`[ChartStyle] Invalid key rejected: ${key.substring(0, 50)}`);
    return null;
  }
...
const ChartStyle = ({ id, config }: { id: string; config: ChartConfig }) => {
  ...
  if (sanitizedId !== id) {
    console.warn(`[ChartStyle] Chart ID was sanitized: ${id} -> ${sanitizedId}`);
  }
